...
export const DEFAULT_SEO_TITLE = "alooSMM 3.0 — Professional SMM Boshqaruv Platformasi";
export const DEFAULT_SEO_DESCRIPTION = "O'zbekistonning eng zamonaviy SMM boshqaruv tizimi. Kontent reja, bonus tizimi, filial hisobotlari, reklama kampaniyalari va AI yordamchi bir joyda.";
...
export function applySeo(settings = null) {
  ...
  const imageUrl = `${siteUrl}/icon-512.svg?v=3.0`;
  const companyName = settings?.company_name ? `${settings.company_name} SMM 3.0` : DEFAULT_SEO_TITLE;

  document.title = companyName;

  upsertMeta("name", "description", DEFAULT_SEO_DESCRIPTION);
  upsertMeta("name", "keywords", "alooSMM, SMM boshqaruv, marketing panel, kontent reja, bonus tizimi, filial nazorat, Uzbekistan SMM");
  ...
  upsertMeta("property", "og:title", companyName);
  upsertMeta("property", "og:description", DEFAULT_SEO_DESCRIPTION);
  upsertMeta("property", "og:site_name", "alooSMM 3.0");
  upsertMeta("property", "og:url", pageUrl);
  upsertMeta("property", "og:image", imageUrl);
  upsertMeta("name", "twitter:card", "summary_large_image");
  upsertMeta("name", "twitter:title", companyName);
